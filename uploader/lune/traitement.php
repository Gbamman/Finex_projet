		...
		if(!in_array($extension, $extensions_valides)){
		 ...
		 /*$fichier = strtr($fichier,'ÀÁÂÃÄÅÇÈÉÊËÌÍÎÏÒÓÔÕÖÙÚÛÜÝàáâãäåçèéêëìíîïðòóôõöùúûüýÿ', 
          'AAAAAACEEEEIIIIOOOOOUUUUYaaaaaaceeeeiiiioooooouuuuyy');
			...
			if(file_exists($dossier.'Encaissement.'.$extension)){
				rename ($dossier.'Encaissement.'.$extension,$dossier.'Encaissement'.date("dmY-h-i-s").'.'.$extension);
				//echo "file existe"; 
			} 
			if(rename ( $fileTmpLoc,$dossier.'Encaissement.'.$extension)){ 
...
$objPHPExcel = PHPExcel_IOFactory::load($dossier.'Encaissement.'.$extension);
