...
if(isset($_FILES["file"]["name"])){
	$response = array();
		...
		$dossier = '../../typepretFolder/imagecoassureur/'; 
		...
		/*	if(file_exists($dossier.'Banque.'.$extension)){
			...
			if(rename ($fileTmpLoc,$dossier.'Coassureur'.$nom.'.'.$extension)){ 
					$nomPermis=chmod($dossier.'Coassureur'.$nom.'.'.$extension, 0644);
				 	$response['status'] = "success";
            		$response['message'] = 'Chargement reussi';
            		$fileNameCallback='Coassureur'.$nom.'.'.$extension;
            	  }
            	}
