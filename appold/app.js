...
app.config(['$routeProvider',
  function ($routeProvider) {
    ...
    when('/login', {
      ...
      title: 'App1',
      templateUrl: 'contrat/vue/dashbord.html',
    ...
    .run(function ($rootScope, $location,Data, $routeParams,$timeout,ActionSurMenue,$cookies,$cookieStore) {
        ...
        Data.put('ModificationUsures/'+utilisateur.uid,{etat:utilisateur.etat});
                 };

                  $rootScope.logout = function (uid) {
  
                      Data.put('logout/'+uid,{table:'user_auth'}).then(function (results) { 
                              ...
                              $rootScope.pseudo = results.pseudo;
                               ...
                               $timeout(function() {});
                                  Data.get('leftMenues').then(function (results) { 
                                               ...
                                               /* $cookieStore.remove('Bonsoir');*/
                                                /*$cookieStore.put('Bonsoir', menus);*/
                                                if (typeof $rootScope.actionsMenue == 'undefined' && $rootScope.uid>0) {
                                            var cheminParDefaut=_.where(menus,{nomcourt:fullRoute.replace('/','')});
