...
$objPHPExcel->getActiveSheet()->setCellValue('T'.$lignedepart,'Num bordereau');
$objPHPExcel->getActiveSheet()->setCellValue('U'.$lignedepart,'Niveau');


$objPHPExcel->getActiveSheet()->getStyle('B'.$lignedepart.':U'.$lignedepart)->getFont()->setName('Arial');
$objPHPExcel->getActiveSheet()->getStyle('B'.$lignedepart.':U'.$lignedepart)->getFont()->setSize(10);

$objPHPExcel->getActiveSheet()->getStyle('B'.$lignedepart.':U'.$lignedepart)->applyFromArray($styleThinBlackBorderOutline1);
$objPHPExcel->getActiveSheet()->getStyle('B'.$lignedepart.':U'.$lignedepart)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
$objPHPExcel->getActiveSheet()->getStyle('B'.$lignedepart.':U'.$lignedepart)->getFill()->getStartColor()->setARGB('F6E8B1');
$objPHPExcel->getActiveSheet()->getStyle('B'.$lignedepart.':U'.$lignedepart)->getfont()->getColor()->setRGB('614B3A');
...
$ligne=$lignedepart;
 ...
 foreach ($response as $key=>$value) 
 {
				...
				$objPHPExcel->getActiveSheet()->setCellValue('M'.$ligne,$response[$key]['primedeces']); // PRIME PERTE EMPLOI
				$objPHPExcel->getActiveSheet()->setCellValue('N'.$ligne,$response[$key]['primeperte']); // PRIME PERTE EMPLOI
				$objPHPExcel->getActiveSheet()->setCellValue('O'.$ligne,$response[$key]['montantsupprime']); // SURPRIME
				...
				$Commission=$primeTotale*0.15;
				$objPHPExcel->getActiveSheet()->setCellValue('P'.$ligne,$response[$key]['accessoires']); // ACCESSOIRES
...
/*				$objPHPExcel->getActiveSheet()->setCellValue('M'.$ligne,$response[$key]['']); // PRIME DECES
				...
				*/
				$objPHPExcel->getActiveSheet()->setCellValue('S'.$ligne,$Commission); // COMMISSION
				$objPHPExcel->getActiveSheet()->setCellValue('T'.$ligne,$response[$key]['numbord']); // NUM BORD
				$objPHPExcel->getActiveSheet()->setCellValue('U'.$ligne,getNiveau($response[$key]['niveau'])); // NIVEAU

// Mise en forme des cellules
 $objPHPExcel->getActiveSheet()->getStyle('B'.$ligne.':U'.$ligne)->getFont()->setName('Arial');
$objPHPExcel->getActiveSheet()->getStyle('B'.$ligne.':U'.$ligne)->getFont()->setSize(10);
$objPHPExcel->getActiveSheet()->getStyle('B'.$ligne.':U'.$ligne)->applyFromArray($styleThinBlackBorderOutline1);

// Format numerique
$objPHPExcel->getActiveSheet()->getStyle('H'.$ligne)->getNumberFormat()->setFormatCode('#,##0');
$objPHPExcel->getActiveSheet()->getStyle('S'.$ligne)->getNumberFormat()->setFormatCode('#,##0');
$objPHPExcel->getActiveSheet()->getStyle('M'.$ligne.':R'.$ligne)->getNumberFormat()->setFormatCode('#,##0');
...
// somme des primes
 ...
 foreach ($list_col as $key => $value) 
 {
	...
	$objPHPExcel->getActiveSheet()->getStyle('M'.($lignedepart-1).':R'.($lignedepart-1))->getNumberFormat()->setFormatCode('#,##0');
	$objPHPExcel->getActiveSheet()->getStyle('M'.($lignedepart-1).':R'.($lignedepart-1))->applyFromArray($styleThinBlackBorderOutline1);
	$objPHPExcel->getActiveSheet()->getStyle('M'.($lignedepart-1).':R'.($lignedepart-1))->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
	$objPHPExcel->getActiveSheet()->getStyle('M'.($lignedepart-1).':R'.($lignedepart-1))->getFill()->getStartColor()->setARGB('F6E8B1');
	$objPHPExcel->getActiveSheet()->getStyle('M'.($lignedepart-1).':R'.($lignedepart-1))->getFont()->setItalic(true);
